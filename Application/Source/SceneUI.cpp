...
{	
	...
	objectlist[hb_HOUSE1].Setuphitbox(Vector3(1, 1, 1), Vector3(0, 3, 0));
				...
				if (checkcollision)
				{
					for (unsigned idx = 0; idx < hb_count; idx++) //check against other hitboxes
					{
						if (!objectlist[idx].getissetup())
							continue;
						else if (objectlist[idx].getobjecthitbox().Checkforcollision(player.getobjecthitbox(), tempplayerhitbox, x, y, z)) //if the player's hitbox is smaller than the object's hitbox
							camera.gotoprevpos();
						else if (player.getobjecthitbox().Checkforcollision(objectlist[idx].getobjecthitbox(), tempplayerhitbox, x, y, z))
							camera.gotoprevpos();
					}
				}
